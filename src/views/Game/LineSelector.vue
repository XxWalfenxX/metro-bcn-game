...
export default {
  ...
  async created() {
    const response = await fetch(
      `https://api.tmb.cat/v1/transit/linies/metro/?app_id=${import.meta.env.VITE_TMB_APP_ID
    ...
    // Borrar Funicular de Montjuic

    const codiLiniaToDelete = 99;

    const indexToDelete = this.lineasMetro.features.findIndex(feature => feature.properties.CODI_LINIA === codiLiniaToDelete);
...
<template>
  <div class="px-4 mx-auto full-screen max-w-screen-xl text-center py-24 lg:py-56">
    ...
    <div class="flex flex-wrap gap-5 justify-center">
      <LineSelectComponent v-for="linea in lineasMetro?.features" v-bind:key="linea.properties.ID_LINIA"
        :name="linea.properties.NOM_LINIA" :color="linea.properties.COLOR_LINIA" :link="linea.properties.CODI_LINIA" />
    </div>
  </div>
</template>
